...
- **`sudo` mit Passwort kann Claude nicht selbst ausführen** (gleicher Grund: kein TTY, und Passwörter grundsätzlich nicht eingeben). Muster: Claude gibt den fertigen Befehl aus, der Nutzer führt ihn in seinem eigenen Terminal aus und meldet "fertig", danach prüft Claude das Ergebnis per SSH. **Korrektur (12.09.2026):** Die früher hier dokumentierte Ausnahme ("`sudo -n reboot` funktioniert passwortlos") galt für die alte Board-1-Karte und ist auf der frischen Board-2-Installation **nicht** mehr der Fall — verifiziert: `sudo -n true` verlangt ein Passwort, es existiert keine NOPASSWD-Regel. Für `main.py`s Power-Button-Shutdown (`power_button.py`) wurde stattdessen eine gezielte NOPASSWD-Regel nur für `systemctl poweroff` angelegt, siehe dortigen Docstring/Abschnitt 8.
...
| ES8388 (Audio-Codec) | **0x10** (verifiziert 12.09.2026 — CE=GND, aber Datenblatt gibt 0x20 vermutlich im 8-Bit-Format an, siehe Abschnitt 7) |
...
- **SX1509** (I2C-GPIO-Expander für Tasten/LEDs) — Hardware-Debouncing, PWM-LED-Treiber integriert. **Keine brauchbare CircuitPython/Blinka-Bibliothek verfügbar** (nur 1-8-Sterne-Hobby-Repos) — Treiber ist rohe I2C-Registeransteuerung per `smbus2`, siehe [Software/buttonbox.py](Software/buttonbox.py). Pin-Layout laut `KiCad/Projekt 1/ButtonBox/ButtonBox.net`: I/O0=Encoder A, I/O1=Encoder B, I/O2=Encoder-Klick, I/O3=Play/Pause, I/O4=Shuffle, I/O5=Stop (alle active-low, interne Pull-ups). I/O7-I/O14 liegen an einem 8-Pin-Reserve-Stecker (J7), noch nicht belegt. **NINT/NRESET sind nicht zum Pi durchverbunden** — Buttons/Encoder müssen gepollt werden, kein Interrupt verfügbar.
  ✅ **Auf Board 2 / ButtonBox verifiziert (09.09.2026):** I2C-Adresse exakt wie vorhergesagt auf **0x3E** (per `i2cdetect`). Play/Pause, Shuffle und Stop liefern über `buttonbox.py` saubere `button_down`/`button_up`-Events ohne Prellen. Encoder-Verkabelung (Pinbezeichnung "J3" ist nur der Netzname, physisch **direkt angelötete Adern, kein steckbarer Connector**): Ader1=Encoder A, Ader2=Encoder B, Ader3=Encoder-Klick, Ader4=VCC, Ader5=GND.
  ✅ **Rotary Encoder — Root Cause gefunden und gefixt (12.09.2026), nach ausführlicher Hardware/Software-Diagnose auf Board 2.** Ursprünglicher Verdacht (Wackelkontakt auf Encoder A) hat sich als falsch herausgestellt: gezieltes Ablöten/Wiederanlöten von A und B einzeln zeigte, dass **beide Leitungen elektrisch einwandfrei sind** (jede toggelt für sich sauber). Auch mehrfaches Nachlöten beider Adern änderte die Erfolgsquote beim langsamen Drehen kaum (~30-40% vollständige Gray-Zyklen vorher wie nachher) — kein Lötstellenproblem. Ein roher, ungefilterter I2C-Capture (2ms-Polling, keine Software-Debounce) auf echter Hardware zeigte ein **komplett sauberes Signal**: alle Klicks durchliefen den vollen 4-Zustands-Gray-Zyklus ohne einen einzigen Bounce/Reversal — aber einzelne Zwischenzustände dauerten teils nur **~2-3ms**. Root Cause: `ENC_DEBOUNCE_S` stand auf 5ms und verlangte 5ms *Ruhe* vor Akzeptanz einer Lesung — ein Zustand, der selbst nur 2-3ms lebt, kann das nie erfüllen und wurde stillschweigend verschluckt. Dazu kam, dass der Encoder in `Software/main.py` nur im 10ms-Render-Loop mitgepollt wurde (`time.sleep(0.01)`), zu grob für 2-3ms-Zustände. **Fix:** `ENC_DEBOUNCE_S` auf 0.001s runtergesetzt (Signal ist nachweislich bounce-frei, braucht nur noch Schutz vor gelegentlichen I2C-Glitches) + neue `ThreadedPoller`-Klasse in `Software/buttonbox.py`, die `ButtonBox.poll()` aus einem eigenen 1ms-Hintergrund-Thread abruft, entkoppelt vom Render-Loop; `main.py` nutzt jetzt `ThreadedPoller(ButtonBox())` statt `ButtonBox()` direkt. Live verifiziert: von ~30-40% auf **18/18 vollständige Klicks, 26 rotate-Events, 0 Richtungswechsel**. `ButtonBox.poll()` selbst blieb synchron (unverändert für die bestehende Test-Suite in `test_buttonbox.py`, alle 14 Tests weiterhin grün).
...
- ✅ **ES8388 (U2) — lebt doch! (12.09.2026, Korrektur zur 11.09.2026-Diagnose):** Antwortete tagelang auf keiner I2C-Adresse — aber wir haben nach der letzten Nachlöt-Runde (Power/GND/SDA/SCL/Exposed-Pad-Via) nie neu gescannt, bevor der Chip als defekt markiert wurde. Beim nächsten Boot-Test zeigte `i2cdetect` plötzlich ein Gerät auf **0x10**, und Register 0x00-0x0b liefern echte, unterschiedliche Werte (0x06, 0x5c, 0xc3, 0xfc, 0xc0, 0x00, 0x00, 0x7c, 0x80, 0x00, 0x00, 0x02) — klar ein lebender Chip, kein Rauschen. **Root Cause der falschen "defekt"-Diagnose:** Wir hatten 0x20 als Zieladresse erwartet (CE=GND laut Datenblatt), aber das Datenblatt gibt die Adresse vermutlich im **8-Bit-Format inkl. R/W-Bit** an — die tatsächliche 7-Bit-Adresse für `i2cdetect`/`smbus2` ist **0x10** (0x20 >> 1). Reg 0x0c liefert einen I/O-Fehler (vermutlich normal, evtl. write-only oder braucht MCLK — noch nicht weiter untersucht). **Für die spätere Treiber-Implementierung: I2C-Adresse ist 0x10, nicht 0x20/0x22.** Kein Ersatzteil nötig. Alle anderen Phase-5-Punkte sind unabhängig davon abgeschlossen.
...
- ✅⚠️ **UI/Menü-State-Machine (11.09.2026 gebaut, seit 12.09.2026 live auf echter Hardware getestet).** Home/Bibliothek/Playlist/Now-Playing (rotierende Cover-Art-Platte mit Tonarm)/Settings (6 Kategorien: Anzeige, LED, Verbindung, Sync, Bewegung, Über)/YouTube-TV-Code-Login/Sync-Fortschritt — siehe `Software/ui_state.py`, `Software/ui_render.py`, `Software/main.py`. Bewegungssensor-Features (Schütteln=Shuffle, Umdrehen=Pause) implementiert, aber **Achsen-Mapping des QMI8658A nur geraten, nicht verifiziert** — siehe `Software/NOTES_FOR_PER.md` für offene Fragen (LED-Kettenlänge, RGB/RGBW, Lautstärke-Regelung, Auto-Sleep). YouTube-Sync-Backend (`Software/sync_youtube.py`) hat einen **echten** Google-OAuth-Device-Flow-Login, aber simuliertes Download-Backend (ytmusicapi/yt-dlp/ffmpeg fehlen noch) — braucht eigene Google-Cloud-OAuth-Zugangsdaten von dir, siehe Docstring dort.
  **Erster Live-Test (12.09.2026) deckte zwei echte Bugs auf, beide inzwischen gefixt:**
  1. ✅ **Display lief nur mit ~1fps.** Root Cause (im tatsächlichen `Adafruit_Blinka_Displayio`-Quellcode auf GitHub verifiziert, nicht nur vermutet): `displayio.OnDiskBitmap` liest jeden Pixel einzeln von der Platte, und `displayio.Bitmap.__setitem__` baut bei jedem einzelnen Pixel-Schreiben ein neues `Area()`-Objekt für die Dirty-Rect-Verwaltung — beides zusammen kostete ~700-900ms pro Frame. **Fix:** `displayio` für die Pixel-Übertragung komplett umgangen — `fourwire.FourWire.send(command, data)` ist eine öffentliche Methode für rohe SPI-Kommandos (CASET/RASET/RAMWR, Standard-MIPI-Befehlssatz wie bei ILI9341/ST7789), genau wie ein Bare-Metal/Arduino-Treiber es machen würde. Siehe `FrameSink` in `Software/main.py`. Jetzt spürbar flüssig.
  2. ✅ **Rotary Encoder unzuverlässig — gefixt**, siehe Abschnitt 3 (SX1509) oben. War kein Hardware-Defekt, sondern Polling/Debounce-Timing (`ENC_DEBOUNCE_S` + 10ms-Render-Loop zu grob für 2-3ms-lange Zwischenzustände) — behoben per `ThreadedPoller` in `Software/buttonbox.py`.
  Volle Recherche-/Debug-Historie inkl. Log-Auszügen: `Software/NOTES_FOR_PER.md`.
...
- **Soft-Shutdown (implementiert 12.09.2026, siehe `Software/power_button.py`):** Power-Taster (GPIO9) wird in `main.py`s Loop gepollt (kein Interrupt, wie bei ButtonBox — einfaches Polling reicht hier). Kurzer Druck (Loslassen vor 5s) = beide Front-LEDs und Frontdisplay-Backlight aus (Toggle, erneuter kurzer Druck schaltet wieder an) — Rückdisplay ist noch nicht in `main.py` integriert (separates Skript, siehe NOTES_FOR_PER.md), bleibt also unberührt. Halten ≥5s = `render_shutdown()` zeigt kurz "Shutting down..." auf dem Frontdisplay, dann `sudo -n systemctl poweroff` → der bestehende Shutdown-Hook zieht GPIO16 (TPS63020 EN) auf LOW (siehe Abschnitt 3). Braucht eine gezielte NOPASSWD-Sudoers-Regel nur für `/usr/bin/systemctl poweroff` (nicht blanket sudo) — einmalig vom Nutzer einzurichten, siehe Docstring in `power_button.py`.
